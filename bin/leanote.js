...
function nedbId() {
  return crypto
    .randomBytes(Math.ceil(Math.max(8, 16 * 2)))
    .toString("base64")
    .replace(/[+\/]/g, "")
    .slice(0, 16);
}
...
function getContext() {
  ...
  if (!fs.existsSync(usersDbPath)) {
    ...
    console.error(
      "Please launch Leanote desktop app first to initialize data.",
    );
    process.exit(1);
  }
  ...
  if (!user) {
    console.error(
      "Error: No user found. Please launch Leanote app and sign in first.",
    );
    process.exit(1);
  }
  ...
  if (!fs.existsSync(userDBPath)) {
    console.error(
      "Error: No data directory for user " +
        userId +
        ". Please launch Leanote app first.",
    );
    process.exit(1);
  }
...
function loadNotes(filter) {
  ...
  return notes
    .filter(function (n) {
      return n.IsTrash !== true && n.LocalIsDelete !== true;
    })
    .filter(
      filter ||
        function () {
...
function findNote(noteId) {
  ...
  for (var i = 0; i < notes.length; i++) {
    if (
      notes[i].NoteId === noteId &&
      notes[i].IsTrash !== true &&
      notes[i].LocalIsDelete !== true
    ) {
      return notes[i];
    }
  }
...
function cmdAdd(args) {
  ...
  for (var ni = 0; ni < notebooks.length; ni++) {
    if (
      notebooks[ni].Title === targetNotebook &&
      notebooks[ni].UserId === ctx.userId
    ) {
  ...
  for (var ti = 0; ti < opts.tags.length; ti++) {
    ...
    if (!found) {
      ...
      updateNDB(
        ctx.tagsPath,
        function (t) {
          return t.UserId === ctx.userId && t.Tag === tagTitle;
        },
        function (t) {
          t.UpdatedTime = now;
          t.LocalIsDelete = false;
          t.IsDirty = true;
          return t;
        },
      );
...
function cmdList(args) {
  ...
  if (opts.notebook) {
    ...
    for (var i = 0; i < notebooks.length; i++) {
      if (notebooks[i].Title === opts.notebook) {
        nbId = notebooks[i].NotebookId;
        break;
      }
    }
    if (nbId) {
      filter = function (n) {
        return n.NotebookId === nbId;
      };
    } else {
  ...
  if (opts.tag) {
    var prev = filter;
    filter = function (n) {
      return prev(n) && n.Tags && n.Tags.indexOf(opts.tag) !== -1;
    };
  }
  ...
  if (count === 0) {
    ...
    console.log(
      "... (" + (notes.length - limit) + " more, use -N to show more)",
    );
  }
}
...
function cmdShow(args) {
  ...
  if (!note) {
    ...
    var notes = loadNotes();
    for (var i = 0; i < notes.length; i++) {
      if (notes[i].NoteId.indexOf(opts.noteId) === 0) {
        note = notes[i];
        break;
      }
    }
  ...
  var tags =
    note.Tags && note.Tags.length > 0 ? note.Tags.join(", ") : "(none)";
...
function cmdEdit(args) {
  ...
  if (Object.keys(fields).length === 0) {
    console.error(
      "Error: No fields to update. Use --title, --content, --tags, or --notebook.",
    );
    process.exit(1);
  }
  ...
  console.log(
    "Updated: " +
      (opts.noteId.length < 20
        ? opts.noteId + "..."
        : opts.noteId.substring(0, 8) + "..."),
  );
}
...
function cmdSearch(args) {
  ...
  if (count === 0) {
    ...
    console.log(
      "... (" + (results.length - limit) + " more, use -N to show more)",
    );
  }
}
...
function printHelp(cmd) {
  if (cmd === "add" || !cmd) {
    ...
    console.log(
      "       leanote [options]                  (add is the default)",
    );
    console.log("");
    ...
    console.log(
      "  -c, --content <content>   Markdown content (use -c or pipe from stdin)",
    );
    console.log("  -g, --tags <tags>         Comma-separated tags");
    console.log(
      '  -n, --notebook <notebook>  Target notebook (default: "AI札")',
    );
    ...
    console.log(
      "  -c, --content <content>   New markdown content (use -c or pipe from stdin)",
    );
    console.log(
      "  -g, --tags <tags>         New comma-separated tags (replaces existing)",
    );
...
function printGlobalHelp() {
  ...
  console.log("");
  console.log("Examples:");
  console.log('  leanote add -t "Note" -c "Content" -g "tag1,tag2"');
    ...
    case "add":
      {
        if (
          pipedContent &&
          !cmdArgs.some(function (a) {
            return a === "-c" || a === "--content";
          })
        ) {
